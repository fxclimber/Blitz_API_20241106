...
UI_Editor::UI_Editor()
{
	...
	if (nullptr == SpriteEdit)
	{
		SpriteEdit = CreateDefaultSubObject<USpriteRenderer>();
		SpriteEdit->SetOrder(ERenderOrder::SURF);
		SpriteEdit->SetSprite("Text_edit_all.png");
		SpriteEdit->SetComponentScale({ 500,62 });
		SpriteEdit->SetComponentLocation({ 400,150 });
		//SpriteEdit->SetPivot({ 0,0 });
		SpriteEdit->SetAlphafloat(0.75f);
...
void UI_Editor::Tick(float _DeltaTime)
{
	...
	if (nullptr == SpriteEdit)
	{
		...
		SpriteEdit->SetOrder(ERenderOrder::SURF);
		SpriteEdit->SetSprite("Text_edit_all.png");
		SpriteEdit->SetComponentScale({ 635,79 });
		SpriteEdit->SetComponentLocation({ 300,300 });
		//SpriteEdit->SetPivot({ 0,0 });
		SpriteEdit->SetAlphafloat(0.75f);
